...
## 2020 ----
concentrado_20 <- read_dta("Bases/ENIGH_20/concentradohogar.dta")
poblacion_20 <- read_dta("Bases/ENIGH_20/poblacion.dta")
...
poblacion1 <- filter(poblacion_20, !(parentesco>=400 & parentesco <500 | 
...
concentrado_20 <- left_join(concentrado_20, poblacion1, by = c("folioviv", "foliohog"))

## Identificación de variables comunes exceptuando las llaves de unión ----
var_rep <- intersect(names(concentrado_20), names(poblacion_20))
var_off <- setdiff(var_rep, c("folioviv", "foliohog"))

conc_pob_20 <- concentrado_20 %>%
  left_join(poblacion_20 %>% select(-all_of(var_off)), 
...
#Ingreso corriente deflactado (base agosto 2020= 100)
conc_pob_20 <- conc_pob_20 %>% mutate (
  ing_total= ing_cor/inpc_20, #ingreso total del hogar real
  ing_trab= ingtrab/inpc_20, #ingreso laboral real
...
educ_prom <- weighted.mean(conc_pob_20$anesc_educ, w= conc_pob_20$factor)
...
tot_hog <- sum(conc_pob_20$factor)

#Tamaño de la distribución:
tam_dec<-trunc(tot_hog/10)

#Incorportando a base:
conc_pob_20$tam_dec=tam_dec

#Creando auxiliar del ingreso:
conc_pob_20$ing_total2 <- conc_pob_20$ing_total

#Ordenando:
conc_pob_20 <- conc_pob_20[with(conc_pob_20, order(rank(ing_total2))),]

#Sea la suma acumulada del factor:
conc_pob_20$ACUMULA <- cumsum(conc_pob_20$factor)

#Bucle para generar los deciles para ingreso total:
for(i in 1:9){
  a1<-conc_pob_20[dim(conc_pob_20[conc_pob_20$ACUMULA<tam_dec*i,])[1]+1,]$factor
  conc_pob_20<-rbind(conc_pob_20[1:(dim(conc_pob_20[conc_pob_20$ACUMULA<tam_dec*i,])[1]+1),],
                  conc_pob_20[(dim(conc_pob_20[conc_pob_20$ACUMULA<tam_dec*i,])[1]+1):dim(conc_pob_20[1])[1],])
  
  b1<-tam_dec*i-conc_pob_20[dim(conc_pob_20[conc_pob_20$ACUMULA<tam_dec*i,])[1],]$ACUMULA
  conc_pob_20[(dim(conc_pob_20[conc_pob_20$ACUMULA<tam_dec*i,])[1]+1),]$factor<-b1
  conc_pob_20[(dim(conc_pob_20[conc_pob_20$ACUMULA<tam_dec*i,])[1]+2),]$factor<-(a1-b1)
}

conc_pob_20$ACUMULA2<-cumsum(conc_pob_20$factor)
conc_pob_20$decil <- 0
conc_pob_20[(conc_pob_20$ACUMULA2<=tam_dec),]$decil <- 1

for(i in 1:9){
  conc_pob_20[((conc_pob_20$ACUMULA2>tam_dec*i)&(conc_pob_20$ACUMULA2<=tam_dec*(i+1))),]$decil <- (i+1)
}

conc_pob_20[conc_pob_20$decil%in%"0",]$decil <- 10
...
#Creando auxiliar del ingreso:
conc_pob_20$ing_trab2 <- conc_pob_20$ing_trab

#Ordenando:
conc_pob_20 <- conc_pob_20[with(conc_pob_20, order(rank(ing_trab2))),]
...
conc_pob_20$ACUMULA2<-cumsum(conc_pob_20$factor)
conc_pob_20$decil_l <- 0
conc_pob_20[(conc_pob_20$ACUMULA2<=tam_dec),]$decil_l <- 1

for(i in 1:9){
  conc_pob_20[((conc_pob_20$ACUMULA2>tam_dec*i)&(conc_pob_20$ACUMULA2<=tam_dec*(i+1))),]$decil_l <- (i+1)
}

conc_pob_20[conc_pob_20$decil_l%in%"0",]$decil_l <- 10
...
write_xlsx(ing_total_tipos, path = ("tablas/2020/ing_total_tipos_2020.xlsx"))
...
# 4. Gráficos ----
## Ingreso total mensual promedio por region 2020 ----
ing_total_nacional <- weighted.mean(conc_pob_20$ing_total, w= conc_pob_20$factor)

library(ggplot2)
library(scales)

G1_ing_total_region_e_20 <- ggplot(ing_total_region_e, aes(x = reorder(region_e, ing_totalp), y = ing_totalp)) +
  # Líneas de error
  ...
  labs(
    title = "Figura 2: ingreso total mensual promedio por región\nen México, 2020",
    subtitle = "Precios de 2024",
    x = "Región",
    y = "Ingreso mensual (miles de pesos)",
    caption = "Fuente: elaboración propia con datos de la ENIGH 2020 de INEGI"
  )

G1_ing_total_region_e_20


# Guardar el gráfico con las dimensiones y formato deseados
ggsave(
  filename = "graficos/G1_ing_total_region_e_20.png", 
  plot = G1_ing_total_region_e_20, 
  width = 10, 
  height = 8, 
  units = "in", 
  dpi = 300,
...
## Ingreso laboral mensual promedio por region 2020 ----
ing_laboral_nacional <- weighted.mean(conc_pob_20$ing_trab, w= conc_pob_20$factor)

library(ggplot2)
library(scales)

G2_ing_trab_region_e <- ggplot(ing_trab_region_e, aes(x = reorder(region_e, ing_trabp), y = ing_trabp)) +
  # Líneas de error
  geom_errorbar(aes(ymin = LCI, ymax = LCS), width = 0.3) +
  # Puntos centrales
  geom_point(color = "firebrick", size = 2.5) +
  ...
  labs(
    title = "Figura 3: ingreso laboral mensual promedio por región\nen México, 2020",
    subtitle = "Precios de 2024",
    x = "Región",
    y = "Ingreso mensual (miles de pesos)",
    caption = "Fuente: elaboración propia con datos de la ENIGH 2020 de INEGI"
...
G2_ing_trab_region_e

# Guardar el gráfico con las dimensiones y formato deseados
ggsave(
  filename = "graficos/G2_ing_trab_region_e.png", 
  plot = G2_ing_trab_region_e, 
  width = 10, 
  height = 8, 
  units = "in", 
  dpi = 300,
